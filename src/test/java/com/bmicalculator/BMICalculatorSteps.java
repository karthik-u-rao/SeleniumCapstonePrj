...
public class BMICalculatorSteps {
    ...
    public void bmi_category_should_be(String expectedCategory) {
        Assert.assertTrue(bmiCalculated, "BMI should be calculated before validating the category");
        Assert.assertNotNull(bmiCategory, "BMI category must not be null");
        Assert.assertEquals(bmiCategory, expectedCategory, "BMI category does not match expected classification");
    }

    @Then("BMI result should be approximately {double}")
    public void bmi_result_should_be_approximately(Double expectedValue) {
        Assert.assertTrue(bmiCalculated, "BMI should be calculated before validating the result");
        Assert.assertNotNull(bmiValue, "BMI value must not be null");
        Assert.assertEquals(bmiValue, expectedValue, 0.1, "BMI result should be within 0.1 of expected value");
    }

    @Then("BMI result should be between {double} and {double}")
    public void bmi_result_should_be_between(Double lowerBound, Double upperBound) {
        Assert.assertTrue(bmiCalculated, "BMI should be calculated before validating the result range");
        Assert.assertNotNull(bmiValue, "BMI value must not be null");
        Assert.assertTrue(bmiValue >= lowerBound, "BMI value is lower than the expected minimum");
        Assert.assertTrue(bmiValue <= upperBound, "BMI value is higher than the expected maximum");
    }
